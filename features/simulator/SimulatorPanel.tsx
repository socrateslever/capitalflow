...
export const SimulatorPanel: React.FC<SimulatorPanelProps> = ({ onClose }) => {
  ...
  const calculation = useMemo(() => {
    ...
    const interest = principal * rate * daysToMaturity;
  ...
  }, [principal, dailyRate, daysToMaturity, installments, isInstallmentMode]);
    ...
    <div className="space-y-6 animate-in fade-in duration-300 font-sans pb-24">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white shrink-0 shadow-lg shadow-blue-900/20">
              <Calculator size={20} />
            </div>
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase mt-1 tracking-widest">Cálculos e Projeções</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 space-y-6 shadow-xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between relative z-10">
            ...
            <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800">
              <button 
                onClick={() => { setIsInstallmentMode(false); resetSimulation(); }}
              ...
              <button 
                onClick={() => { setIsInstallmentMode(true); resetSimulation(); }}
                className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${
                  isInstallmentMode 
                    ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' 
                    : 'text-slate-500 hover:text-slate-300'
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 space-y-6 shadow-2xl relative overflow-hidden">
          ...
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2 relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 relative z-10">
            <div className="bg-slate-950/50 rounded-2xl p-5 border border-slate-800/50">
              <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-1">Juros Totais</p>
              <p className="text-xl font-black text-white">{formatMoney(calculation.interest)}</p>
            </div>
            <div className="bg-slate-950/50 rounded-2xl p-5 border border-slate-800/50">
              <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-1">Custo Efetivo (CET)</p>
            ...
            <div className="bg-blue-600/10 rounded-2xl p-5 border border-blue-500/20">
              <p className="text-[9px] text-blue-400 font-black uppercase tracking-widest mb-1">Valor Final Total</p>
          ...
          {isInstallmentMode && (
            <div className="space-y-4 relative z-10">
              <div className="bg-slate-950/80 rounded-2xl p-5 border border-slate-800 flex items-center justify-between">
                <div>
                  <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Valor de cada Parcela</p>
                  <p className="text-2xl font-black text-white">{formatMoney(calculation.installmentValue)}</p>
                </div>
                <div className="text-right">
                  <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Frequência</p>
                  <p className="text-xs font-black text-white uppercase">A cada {(daysToMaturity / installments).toFixed(0)} dias</p>
                </div>
              </div>

              {/* Lista de Parcelas */}
              <div className="space-y-2">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Cronograma de Pagamentos</p>
                <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {calculation.installmentList.map((inst) => (
                    <div key={inst.number} className="flex items-center justify-between bg-slate-950/40 p-3 rounded-xl border border-slate-800/50 hover:bg-slate-950 transition-colors">
                      <div className="flex items-center gap-3">
                        <span className="w-6 h-6 rounded-lg bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-400">
                          {inst.number}
                        </span>
                        <span className="text-xs font-bold text-slate-300">{formatDate(inst.dueDate)}</span>
                      </div>
                      <span className="text-xs font-black text-white">{formatMoney(inst.value)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {!isInstallmentMode && (
            <div className="bg-slate-950/80 rounded-2xl p-5 border border-slate-800 flex items-center justify-between relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Vencimento Único</p>
                  <p className="text-sm font-black text-white">{formatDate(calculation.dueDate)}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Prazo</p>
          ...
          {hasSimulated && simulationResult && (
            <div className="bg-slate-950 rounded-3xl p-6 border border-blue-500/30 space-y-5 animate-in zoom-in-95 duration-300 relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 text-blue-400">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">Resultado Projetado</span>
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Saldo Devedor Remanescente</p>
                ...
                {simulationResult.nextInstallmentDate && (
                  <div className="space-y-2">
                    <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Próximo Vencimento</p>
                    <p className="text-2xl font-black text-white">
